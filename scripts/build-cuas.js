...
const TEMPLATE_REPLACEMENTS = [
  ...
  [
    ...
    "    mapRef.current = map;\n\n    // Wheel policy (capture phase, runs before Mapbox's own canvas handler):\n    //   - ordinary two-finger trackpad / mouse-wheel scroll  -> page scrolls,\n    //     map ignores the gesture entirely.\n    //   - pinch (browsers report this as wheel + ctrlKey=true) -> mapbox zooms,\n    //     subject to the minZoom floor below.\n    const onWheelCapture = (e) => {\n      if (!e.ctrlKey) {\n        // Not a pinch: hand the wheel back to the document so the briefing\n        // keeps scrolling and the map stays put.\n        e.stopImmediatePropagation();\n        return;\n      }\n      if (!mapRef.current) return;\n      const z = mapRef.current.getZoom();\n      const min = mapRef.current.getMinZoom();\n      if (e.deltaY > 0 && z <= min + 0.01) {\n        // Pinch out at the floor: let the page scroll instead of fighting it.\n        e.stopImmediatePropagation();\n      }\n    };\n    containerRef.current.addEventListener('wheel', onWheelCapture, { capture: true, passive: true });\n\n    // Sync state when user drags/rotates/pitches"
  ],
  [
